Router.map(function () {
	this.route('solo', {
		...
		data: function () {
			...
			var tasksByTag = {
				default: Tasks.find({group: 'default', tag: 'default'}, {sort: {name: 1}}).fetch()
			};
				returnData.tags =  foundTags;


				foundTags.forEach(function (tag) {
					tasksByTag[tag._id] = Tasks.find({group: 'default', tag: tag._id}, {sort: {name: 1}}).fetch();
				});
			...
			returnData.groups = Groups.find().fetch();
	...
	this.route('groupMember', {
		...
		data: function () {
			...
			if (group) {
				...
				returnData.user = this.params.userId;
				return returnData;
			}
	...
	this.route('group', {
		...
		data: function () {
			...
			if (group) {
				...
				var foundTags = Tags.find({group: group._id}, {sort: {tasks: -1, name:1}});
				...
				var tasksByTag = {
				default: Tasks.find({group: group._id, tag: 'default'}, {sort: {name: 1}}).fetch()
				};
					returnData.tags =  foundTags;

					foundTags.forEach(function (tag) {
						tasksByTag[tag._id] = Tasks.find({group: group._id, tag: tag._id}, {sort: {name: 1}}).fetch();
					});
