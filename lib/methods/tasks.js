Tasks.methods.newTask = new ValidatedMethod({
  ...
  run (newTask) {
    ...
    const groups = Roles.getGroupsForUser(this.userId);
    if (groups[0] != group._id) {
      throw new Meteor.Error(
...
Tasks.methods.editTask = new ValidatedMethod({
  ...
  validate: new SimpleSchema({
    ...
    'modifier.tagId': { type: String },
    'modifier.name': { type: String },
    'modifier.period': { type: Number },
    'modifier.scheduleType': { type: String },
    'modifier.due': { type: Date },
    'modifier.weight': { type: Number },
  ...
  run ({ _id, modifier }) {
    ...
    const tag = Tags.findOne(modifier.tagId);
    if (!tag) {
      throw new Meteor.Error(
    ...
    if (Tasks.findOne({_id: {$ne: _id}, groupId: modifier.groupId, name: modifier.name})) {
      throw new Meteor.Error(
        'Tasks.methods.nameExists',
        'Task names must be unique per group',
        'Group already has task named: '+modifier.name
      );
    }
    return Tasks.update(_id, {$set: modifier});
  }
});

Tasks.methods.deleteTask = new ValidatedMethod({
